...
export class PostitComponent implements OnInit {
  ...
  itemRenderer = (element: any, item: any, resource: any): void => {
    ...
    )[0].innerHTML = `<div style=" padding-top: 5px; margin-left: 2px">${
      item.text
    }</div>`;
    element[0].getElementsByClassName(
      "jqx-kanban-item-color-status"
    ...
    )[0].innerHTML = `<div style=" margin-top: 2px; margin-left: 2px">${
      item.content
    }</div>`;
  };
  ...
  columnRenderer: any = (
    ...
    if (element[0]) {
      if (
        element[0].getElementsByClassName("jqx-kanban-column-header-title")[0]
          .innerHTML !== "Backlog"
      ) {
      }
    }
  ...
  constructor(
    ...
    this.databaseService.changes.subscribe(change => {
      if (change.doc._deleted) {
        this.myKanban.removeItem(change.doc._id);
      } else {
        if (
          change.type === "Ressource application" &&
  ...
  ngOnInit(): void {
    this.appsService.getRessources(this.appId).then(res => {
      ...
      for (const element of appRessources["docs"]) {
        if (
          element.ressourceType === "Column" &&
          element.application === this.appId
        ) {
          const column = {
            text: element.name,
