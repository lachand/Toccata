  ...
  ngOnInit(): void {
    this.activityService
      ...
      .then(activityInfos => {
        ...
        this.activityService
          .getActivityInfos(activityInfos["currentLoaded"])
          .then(currentLoadedInfos => {
    ...
    this.activityService.changes.subscribe(change => {
      if (change.type === "Activity" && change.doc._id === this.activityId) {
        this.activityService
          ...
          .then(activityInfos => {
            ...
            this.activityInfos = activityInfos;
            this.activityService
              .getActivityInfos(this.activityInfos["currentLoaded"])
              .then(currentLoadedInfos => {
                this.currentLoadedInfos = currentLoadedInfos;
                this.currentLoadedName = currentLoadedInfos["name"];
      ...
      if (
        !isNullOrUndefined(this.activityInfos) &&
        change.doc._id === this.activityInfos["currentLoaded"]
      ) {
        this.activityService
          .getActivityInfos(this.activityInfos["currentLoaded"])
          .then(currentLoadedInfos => {
