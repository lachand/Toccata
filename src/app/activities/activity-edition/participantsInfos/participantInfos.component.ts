...
export class ParticipantInfosComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.userService.getParticipantInfos(this.participantId).then(res => {
      ...
      this.avatarUrl = this.sanitizer.bypassSecurityTrustResourceUrl(
        this.participant["url"]
      );
      console.log(this.participant);
      this.participant_name = `${this.participant["name"][0]} ${
        this.participant["surname"][0]
      }`;
      this.participant_full_name = `${this.participant["name"]} ${
