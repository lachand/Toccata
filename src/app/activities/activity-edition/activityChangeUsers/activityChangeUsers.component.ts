...
export class ActivityChangeUsersComponent {
  ...
  reloadUsers() {
    return this.activityService.user.getAllusers().then(() => {
      ...
      for (const user of this.activityService.user.allUsers) {
        if (
          user.activites.indexOf(this.activityService.activityLoaded._id) !== -1
        ) {
          this.userChecked.push({ user: user, checked: true });
          this.userCheckedInitially.push({ user: user, checked: true });
        } else {
  ...
  changeUsers() {
    ...
    return Promise.all(
      this.userChecked.map(userSelected => {
        for (let i = 0; i < this.userCheckedInitially.length; i++) {
          console.log(userSelected);
          if (
            userSelected.checked &&
            userSelected.user === this.userCheckedInitially[i].user
          ) {
            userList.push(userSelected.user);
          }
          if (
            userSelected.user === this.userCheckedInitially[i].user &&
            userSelected.checked !== this.userCheckedInitially[i].checked
          ) {
            const user = userSelected.user;
            if (userSelected.checked) {
              return this.userService
                .addActivity(this.activityService.activityLoaded.parent, user)
                .then(() => {
                  this.logger.log(
                    "UPDATE",
      ...
      return this.activityService.editParticipants(
        userList,
